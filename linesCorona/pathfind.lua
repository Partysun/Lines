...
function CalcMoves(board, startX, startY, targetX, targetY)
	...
	while listk > 0 do
		...
		if curSquare.x + 1 <= xsize and board[curSquare.x+1][curSquare.y].isObstacle ~= 1 then
			rightOK = false
		end
		if curSquare.x - 1 >= 1 and board[curSquare.x-1][curSquare.y].isObstacle ~= 1 then
			leftOK = false
		end
		if curSquare.y + 1 <= ysize and board[curSquare.x][curSquare.y+1].isObstacle ~= 1 then
			downOK = false
		end
		if curSquare.y - 1 >= 1 and board[curSquare.x][curSquare.y-1].isObstacle ~= 1 then
			upOK = false
		end
